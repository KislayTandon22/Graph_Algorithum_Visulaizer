...
const Graph: React.FC<GraphProps> = ({ nodes, links }) => {
  ...
  useEffect(() => {
    ...
    // Update the positions based on the simulation
    simulation.on('tick', () => {
      link
...
const App: React.FC = () => {
  ...
  const [segmentNumber, setSegmentNumber] = useState<number | null>(null);  // Store segment number
  const [lineCoverNumber, setLineCoverNumber] = useState<number | null>(null);  // Store line cover number
  const [nodeInput, setNodeInput] = useState('');
  const [linkSource, setLinkSource] = useState('');
  const [linkTarget, setLinkTarget] = useState('');

  // Function to calculate segment number
